...
The OpenAI-compatible `/v1/models` shape does not standardize Pi thinking-level / reasoning-effort metadata. Therefore discovery first infers efforts from the explicit model ID suffix whitelist `-none`, `-low`, `-medium`, `-high`, `-xhigh`, and `-max`. Pi `off` is represented as `null` so no provider effort is sent, Pi `minimal` maps to provider effort `low`, and `max` remains distinct from `xhigh`. After suffix inference, the extension probes a supplemental OmniRoute endpoint that may expose richer reasoning-effort metadata.
